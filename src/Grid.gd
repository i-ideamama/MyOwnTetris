...
func new_piece():
	...
	var check_block_pos_for_game_over
	
	for b in current_block_list:
		check_block_pos_for_game_over = world_to_map(b.position)
		if grid[check_block_pos_for_game_over.x][check_block_pos_for_game_over.y] == POS_TYPE.OCCUPIED:
			game_over = true
...
func clear_row():
	...
	for y in rows_to_be_cleared:
		...
		for b in get_children():
			if b is Sprite:
				b_grid_pos = world_to_map(b.position)
				if b_grid_pos.y < y and b_grid_pos.y < 19:
					grid[b_grid_pos.x][b_grid_pos.y] = POS_TYPE.EMPTY
					b.position.y += 32
					b_grid_pos.y += 1
					if not b in current_block_list:
						grid[b_grid_pos.x][b_grid_pos.y] = POS_TYPE.OCCUPIED
	
	
	rows_cleared_at_once = len(rows_to_be_cleared)
	if rows_cleared_at_once == 1:
		score += 40
	elif rows_cleared_at_once == 2:
		score += 100
	elif rows_cleared_at_once == 3:
		score += 300
	elif rows_cleared_at_once == 4:
		score += 1200
	Score_count.text = str(score)
	
	pause_new_piece = false
	cleared_line_count += 1
	Lines_count.text = str(cleared_line_count)
	rows_to_be_cleared = []
